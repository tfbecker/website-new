...
The annoying part is that every register hides public data behind a different wall: CAPTCHAs, session-bound encrypted payloads, JSF state machines, anti-bot walls, PDFs. So I reverse-engineered the official portals — Bundesanzeiger, Unternehmensregister, Handelsregister, INPI (France), KRS (Poland) and Companies House (UK) — and made them all return clean JSON.

Two things I'm happy with on the German side:

- It handles the **DiRUG** split. Annual accounts for fiscal years after 2021 moved from the Bundesanzeiger to the Unternehmensregister, so most scrapers out there are ~3 years stale. This one queries both sources in parallel and routes each year to whichever one actually has it.
